...
const updateProduct = async (id) => {
    ...
    const showCom = () => {
        return (products.result ?
            <Box>
                <Typography variant={'h5'} sx={{ marginTop: '2em' }}>No Movies found...</Typography>
            </Box>
            :
            <Box>
                            ...
                            products.map((e, index) => {
                                ...
                                return (<Fragment key={e._id}>
                                    <TableRow  >
                                        ...
                                        <Link
                        to='/video'
                        state={{ videoTitle: e.filename,favourtie: e.isFavourite }} // <-- state prop
                        key={e.name}
                      >
                                        <video
                                        ...
                                        <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={6}>
                                            <Collapse in={open && rowNo === index} timeout="auto" unmountOnExit>
                                                <Box sx={{ margin: 1, textAlign: 'center' }}>
                                                    ...
                                                    <TextField variant='outlined' color='secondary' type={'text'} onChange={(e) => setGenre(e.target.value)} value={genre} label='Genre' sx={sty} required />
